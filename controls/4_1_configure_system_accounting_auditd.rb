...
control 'cis-dil-benchmark-4.1.12' do
  ...
  command('find -O3 / -xdev \( -perm -4000 -o -perm -2000 \) -type f').stdout.split.map { |x| "^-a (always,exit|exit,always) -F path=#{x} -F perm=x -F auid>=#{uid_min} -F auid!=4294967295 -k privileged$" }.each do |entry|
    describe file('/etc/audit/audit.rules') do
